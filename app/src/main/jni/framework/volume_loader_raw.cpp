...
Volume_loader_raw::load_volume(std::string filepath) {
    SDL_RWops *rw;
    rw = SDL_RWFromFile(filepath.c_str(), "rb");
...
//        volume_file.close();
        ...
        SDL_RWread(rw, (char *) &data.front(), 1, data_size);
        SDL_RWclose(rw);
...
glm::ivec3 Volume_loader_raw::get_dimensions(const std::string filepath) const {
    ...
    if (p1 != p0) {
        ...
        width = (unsigned int) std::atoi(token.c_str());
    }
    ...
    if (p1 != p0) {
        std::string token = filepath.substr(p0, p1 - p0);
        height = (unsigned int) std::atoi(token.c_str());
    }
    ...
    if (p1 != p0) {
        std::string token = filepath.substr(p0, p1 - p0);
        depth = (unsigned int) std::atoi(token.c_str());
    }
...
unsigned Volume_loader_raw::get_channel_count(const std::string filepath) const {
    ...
    std::string token = filepath.substr(p0, p1 - p0);
    channels = (unsigned int) std::atoi(token.c_str());
...
unsigned Volume_loader_raw::get_bit_per_channel(const std::string filepath) const {
    ...
    std::string token = filepath.substr(p0, p1 - p0);
    byte_per_channel = (unsigned int) std::atoi(token.c_str());
